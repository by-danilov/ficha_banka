...
if not any(isinstance(handler, logging.FileHandler) and handler.baseFilename.endswith('analytics.log') for handler in logger.handlers):
    import os
    log_dir = 'logs'
    os.makedirs(log_dir, exist_ok=True)
    file_handler = logging.FileHandler(os.path.join(log_dir, 'analytics.log'), mode='w', encoding='utf-8')
    formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    file_handler.setFormatter(formatter)
    logger.addHandler(file_handler)


def get_transactions_by_date(transactions: list[dict], target_date_str: str) -> list[dict]:
    """
    ...
    try:
        ...
        for transaction in transactions:
            try:
                ...
                logger.warning(f"Некорректный формат даты в транзакции: {transaction.get('date')}. Пропущена.")
                continue
            except TypeError:
                logger.warning(f"Тип данных даты в транзакции некорректен: {transaction.get('date')}. Пропущена.")
                continue
        logger.info(f"Найдено {len(filtered_transactions)} транзакций за {target_date_str}.")
    except ValueError:
        logger.error(f"Некорректный формат целевой даты: '{target_date_str}'. Ожидается ДД.ММ.ГГГГ.")
...
def get_card_number_masked(card_number: str) -> str:
    ...
    if not isinstance(card_number, str) or not card_number.strip():
        logger.warning(f"Получен некорректный номер карты для маскировки: '{card_number}'")
    ...
    for part in parts:
        if part.isdigit() and len(part) == 16: # Предполагаем, что полный номер карты - 16 цифр
            number_part = part
        elif part.isdigit() and len(part) == 19 and part.count('*') > 0: # Может быть уже частично маскирован
            number_part = part.replace('*', '') # Убираем звездочки для корректного маскирования
        elif part.isdigit() and (len(part) == 4 or len(part) == 6): # Если номер уже разбит на части
            number_part += part
        else:
            name_parts.append(part)

    if not number_part: # Если номер не найден в явном виде, попробуем обработать всю строку
        cleaned_number = ''.join(filter(str.isdigit, card_number))
        if len(cleaned_number) >= 16:
            number_part = cleaned_number[-16:] # Берем последние 16 цифр, если их больше
            name_parts = [card_number.replace(number_part, '').strip()]
        else:
            logger.warning(f"Номер карты слишком короткий для маскировки: '{card_number}'")
    ...
    if len(number_part) >= 16: # Требуется минимум 16 цифр для стандартного маскирования
        masked_number = f"{number_part[:4]} {number_part[4:6]}** **** {number_part[-4:]}"
    elif len(number_part) >= 10: # Если номер короче 16, но достаточно длинный для частичной маскировки
...
def get_account_number_masked(account_number: str) -> str:
    ...
    if not isinstance(account_number, str) or not account_number.strip():
        logger.warning(f"Получен некорректный номер счета для маскировки: '{account_number}'")
    ...
    if "Счет" not in account_number:
        logger.debug(f"Маскирование счета: '{account_number}' -> 'Счет {masked_number}'")
        ...
        if len(parts) > 1 and parts[0] == 'Счет':
            ...
            logger.debug(f"Маскирование счета: '{account_number}' -> '{account_number.replace(cleaned_number, masked_number)}'")
...
if __name__ == '__main__':
    ...
    sample_transactions = [
        {'id': 1, 'date': '14.03.2023', 'description': 'Оплата', 'amount': 100, 'currency': 'RUB'},
        {'id': 2, 'date': '15.03.2023', 'description': 'Перевод', 'amount': 200, 'currency': 'USD'},
        {'id': 3, 'date': '14.03.2023', 'description': 'Покупка', 'amount': 50, 'currency': 'RUB'}
    ]
    ...
    print(f"Visa Platinum 7000792296156361: {get_card_number_masked('Visa Platinum 7000792296156361')}")
    print(f"Maestro 781084** **** 5568: {get_card_number_masked('Maestro 781084******5568')}")
    print(f"7810841234565568: {get_card_number_masked('7810841234565568')}")
    print(f"American Express 123456789012345: {get_card_number_masked('American Express 123456789012345')}") # Менее 16 цифр
    ...
    print(f"Счет 40812345678901234506: {get_account_number_masked('Счет 40812345678901234506')}")
