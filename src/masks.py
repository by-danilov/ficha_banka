...
def mask_card_number(card_number):
    ...
    logger.debug(f"Замаскирован номер карты: '{masked_number}' (исходный: '{card_number[:6]}...{card_number[-4:]}')")
    return masked_number

def mask_account_number(account_number):
    """Маскирует номер счета, оставляя видимыми только последние 4 цифры."""
    if not isinstance(account_number, str) or not re.match(r'^\d{10}$', account_number):
        logger.error(f"Попытка маскировки некорректного номера счета: '{account_number}'")
        return "Некорректный номер счета"
    masked_part = '*' * (len(account_number) - 4)
    masked_number = masked_part + account_number[-4:]
    logger.debug(f"Замаскирован номер счета: '{masked_number}' (исходный: '{account_number[:3]}...{account_number[-4:]}')")
    return masked_number
